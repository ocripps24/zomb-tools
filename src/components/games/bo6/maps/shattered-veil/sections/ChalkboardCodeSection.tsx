	...
	return (
		<BaseSection
			config={{
				...
				description:
					"Select your chalkboard and enter the codeword from the dot-matrix printer to generate the code.",
				resetButtonText: "Reset Chalkboard Code",
			}}
			getProgress={(data: ChalkboardData) => {
				const hasChalkboard = Boolean(data.selectedChalkboard);
				const hasCodeword = Boolean(
					data.selectedCodeword && data.selectedCodeword.trim()
				);
		...
		>
			{({ data, setData, progress }) => {
				...
				const getFinalCode = () => {
					...
					return chalkboard.solutions[
						data.selectedCodeword as keyof typeof chalkboard.solutions
					];
				};
					...
					<div className="chalkboard-code-section">
						...
						<div className="chalkboard-selection">
							...
							<div className="chalkboard-grid">
								{Object.entries(CHALKBOARD_SOLUTIONS).map(
									([id, chalkboard]) => (
										<div
											key={id}
										...
										>
											<div className="chalkboard-image">
												<img
													src={
														CHALKBOARD_IMAGES[
															parseInt(id) as keyof typeof CHALKBOARD_IMAGES
														]
													}
													alt={`Chalkboard ${id}`}
													loading="lazy"
						...
						<div className="codeword-selection">
							...
							<p className="selection-description">
								Enter the codeword obtained from the dot-matrix printer in the
								East Hallway after inserting the floppy disk.
							</p>
						...
						{finalCode && (
							<div className="code-summary">
								<ResultsDisplay
									...
									finalCode={finalCode}
									codeFormat="standard"
									codeNote="Use this 4-digit code to unlock the Doppleghast chamber."
									progressMode="badge"
									progress={{
